...
for ELEMENT in ${DISTROS[@]}; do
    [[ -d $ELEMENT ]] && docker run -t centos sh ls / || exit
    containerID=$(docker container ls -a | grep -i $ELEMENT | awk '{print $1}') || exit
    docker export $containerID > $TAR_DIR/$ELEMENT-$(date +%d%m%y).tar || exit
